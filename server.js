...
app.get("/events", isLoggedIn, async (req, res) => {
  ...
  for (let ev of events) {
    if (ev.date.toJSON().slice(0, 10) > date.toJSON().slice(0, 10)) {
      ...
      if (
        parseInt(ev.start.split(":")[0]) <= parseInt(date.getHours()) &&
        parseInt(date.getHours()) < parseInt(ev.end.split(":")[0]) &&
        parseInt(ev.start.split(":")[1]) <= parseInt(date.getMinutes())
      ) {
        progress.push(ev);
      } else {
        if (
...
app.post("/events/add", (req, res) => {
  ...
  if (
    ...
    (parseInt(start.split(":")[0]) == parseInt(end.split(":")[0]) &&
      parseInt(start.split(":")[1]) > parseInt(end.split(":")[1]))
  ) {
...
app.get("/calender/add/:id", async (req, res) => {
  ...
  if (register.length) {
    for (let r of register) {
      if (
        event.date.getDate() == r["date"].getDate() &&
        parseInt(event.start.split(":")[0]) >= parseInt(r["start"]) &&
        parseInt(event.end.split(":")[0]) <= parseInt(r["end"])
      ) {
        allow = false;
        req.flash(
          "success",
          "Sorry, You cannot add this event. You have another event schedulted on this time"
        );
        return res.redirect("/events");
      }
    }
